...
The group is done when the repo is green: run the project's green command from `specs/tech-stack.md`. Verify behavior for real where practical (run the app, hit the routes), not just via unit tests. If the gate goes red, diagnose the root cause before changing anything — no retry-until-green. Commit on main with the message format the plan prescribes.
